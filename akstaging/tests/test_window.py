...
class MockAdwApplicationWindow(MockGObjectClass):
    pass
...
class MockGtkWidget(MockGObjectClass):
    pass
...
class MockGtkWindow(MockGtkWidget):
    pass
...
class MockGtkApplicationWindow(MockGtkWindow):
    pass
...
mock_defs = MagicMock()
mock_defs.APP_NAME = "TestApp"
mock_defs.COPYRIGHT = "TestCopyright"
mock_defs.RESOURCE_PATH = "/test/resource/path"
mock_defs.VERSION = "0.0.test"
sys.modules["akstaging.defs"] = mock_defs

mock_dns_utils_module = MagicMock()
mock_DNSUtils_class = MagicMock()
...
class TestDataObject:
    ...
    def __eq__(self, other):
        if not isinstance(other, TestDataObject):
            return NotImplemented
        return self.ip == other.ip and self.hostname == other.hostname

    def __repr__(self):
        return f"TestDataObject(ip='{self.ip}', hostname='{self.hostname}')"
...
class TestAkamaiStagingWindow(unittest.TestCase):
    ...
    def test_populate_store(self):
        ...
        self.assertFalse(
            isinstance(AkamaiStagingWindow, MagicMock),
            f"AkamaiStagingWindow is a {type(AkamaiStagingWindow)}, expected a class.",
        )
        self.assertTrue(
            callable(AkamaiStagingWindow.populate_store),
        ...
        hosts_content = [
            "1.2.3.4 example.com",
            "5.6.7.8",
            "# 9.10.11.12 commented.com",
            "",
            "127.0.0.1 localhost",
            "10.0.0.1 mydockercontainer",
            "4.3.2.1 another.example.com",
        ]
        ...
        with (
            patch("akstaging.window.DataObject", new=TestDataObject),
            patch("akstaging.window.logger", MagicMock()),
    ...
    def test_delete_entry_permission_error(self):
        permission_msg = (
            "Permission denied when trying to read or write to /etc/hosts. Root privileges may be required."
        )
    ...
    def test_get_ip_permission_error(self):
        ...
        with (
            ...
            window_instance.on_get_ip_button_clicked(
                None, window_instance.entry_domain, window_instance.textview_status
            )

            mock_akl_instance.sanitize_domain.assert_called_once_with("example.com", window_instance.textview_status)
            window_instance.ns.get_akamai_staging_ip.assert_called_once_with(
                "example.com", window_instance.textview_status
            )
