...
def parse_common_arguments():
    ...
    parser.add_argument(
        "command", choices=["update", "remove", "read"], help="The operation to perform: 'update', 'remove', or 'read'."
    )
    parser.add_argument(
        "--ip", type=str, required=False, help="The IP address. Required for 'update' and 'remove' operations."
    )
    parser.add_argument(
        "--domain", type=str, required=False, help="The domain name. Required for 'update' and 'remove' operations."
    )
    parser.add_argument(
        "--delete",
        type=str,
...
def handle_read_command(log_func):
    ...
    try:
        ...
        print(
            f"{Status.ERROR_PERMISSION.name}:Permission error reading hosts file (helper should be root): {e_perm_read}"
        )
        sys.exit(0)
