...
try:
    ...
    if "_gi" in str(e_import):  # Expected if Gtk components are hit
        ...
        print(
            "This part of test considered 'conditionally passed' as no new pre-gi-import errors were found for akstaging.window chain."
        )
    elif (
        "LOCALEDIR" in str(e_import).upper()
        or "APP_ID" in str(e_import).upper()
        or "LOCALE_DIR" in str(e_import).upper()
    ):  # Made case-insensitive for LOCALE_DIR
